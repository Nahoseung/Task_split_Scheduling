...
float Sum_of_H_priority(int P_num, int tail_priority)
{
    ...
    while(temp_i >=0)
    {
        if( P->list[temp_i]->priority < tail_priority){
            sum+= P->list[temp_i]->Utilization;
            // printf("Im gonna add (%d,%d) %f\n",P->list[temp_i]->priority,P->list[temp_i]->sub_num,P->list[temp_i]->Utilization);
        } 
        temp_i --;
    }
    return sum;
...
bool Tail_Schedulability()
{
    ...
    for(int i=1; i< Num_of_T+1 ; i++)
    {
        if(task_manager[i])
        {
            ...
            // printf("IM Tail task(%d,%d) IM in Processor %d  and my H_sum is %f  \n",T->priority,T->sub_num,T->Assigned_P,H_sum);
