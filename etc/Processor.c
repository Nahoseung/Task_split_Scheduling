...
processor* Pop_processor(processor* processor_stack_ptr[])
{
    ...
    {
        if(processor_stack_ptr[i]->Utilization < processor_stack_ptr[min_i]->Utilization)
        {
            min_i = i;
        }
    }
    return processor_stack_ptr[min_i];
}
...
void init_processor(processor* processor_stack_ptr[])
{
    ...
    {
        processor_stack_ptr[i]->Pnum =i;
        processor_stack_ptr[i]->Utilization = 0.0f;
        init_task_stack(processor_stack_ptr[i]->Wait_Q);
    }
}
